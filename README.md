## Questions

1. How much data your publisher program will send to the message broker in one run?

The publisher program transmits five distinct events to the message broker during a single execution. Each event is formatted as a 'UserCreatedEventMessage' that contains a unique user ID and a username tagged with my NPM. This data volume is achieved by explicitly calling the publish_event method five times within the code. Once sent, these messages are stored within the "user_created" queue on the RabbitMQ server. The messages remain in the queue until an active subscriber is ready to consume and process them. This rapid delivery of five data packets creates the characteristic activity spikes visible on the RabbitMQ management dashboard.

2. The url of: “amqp://guest:guest@localhost:5672” is the same as in the subscriber program, what does it mean?

The identical connection URL ensures that both publisher and subscriber are talking to the exact same centralized message broker instance. The string "amqp://guest:guest@localhost:5672" provides the broker's protocol, security credentials, and network address. By using localhost, we are telling the Rust program that the RabbitMQ service is running right on the machine inside a Docker container. The port 5672 acts as the specific technical mailbox where the broker listens for these AMQP messages. Both programs point to this same address. Hence, the publisher knows exactly where to drop the events and the subscriber knows exactly where to find them.

## Running RabbitMQ
<img width="1582" height="959" alt="Screenshot 2026-05-11 at 12 25 32" src="https://github.com/user-attachments/assets/2975b916-a7b8-4624-a0a1-7c5bdc6e4e7f" />
<img width="1582" height="959" alt="Screenshot 2026-05-11 at 12 25 47" src="https://github.com/user-attachments/assets/da14f37e-b8e7-44a3-a963-f2567ce569bf" />
<img width="1582" height="959" alt="Screenshot 2026-05-11 at 12 25 58" src="https://github.com/user-attachments/assets/cc7d731a-fb32-4e5a-8884-154da19a2575" />
<img width="1382" height="832" alt="Screenshot 2026-05-11 at 12 26 59" src="https://github.com/user-attachments/assets/47e5f282-eb2f-4191-b033-7c3d445bbe10" />

## Sending and Processing Event
<img width="985" height="166" alt="Screenshot 2026-05-11 at 12 35 32" src="https://github.com/user-attachments/assets/68d5bd41-e39b-466c-bee7-9e50ea65a49b" />
<img width="1126" height="294" alt="Screenshot 2026-05-11 at 12 35 19" src="https://github.com/user-attachments/assets/645cc330-6d79-476e-ac6c-f8adf7a505a3" />

When you execute cargo run in the publisher directory, the program creates five distinct UserCreatedEventMessage objects and sends them to the RabbitMQ message broker at "localhost:5672". These events are not sent directly to the subscriber. Instead, RabbitMQ acts as a middleman, placing these messages into a queue named user_created. The spikes in the RabbitMQ "Message rates" chart represent the moments the publisher "published" these batches of data to the exchange. On the other side of the system, the subscriber program is running a continuous loop, listening for any new messages that appear in that specific queue. As soon as RabbitMQ receives the events from the publisher, it delivers them to the connected subscriber. The subscriber then triggers the handle function, which prints the confirmation message to the terminal console for each of the five events received. This demonstrates a successful asynchronous communication flow where the publisher and subscriber operate independently through a broker.

## Monitoring chart
<img width="1582" height="959" alt="Screenshot 2026-05-11 at 12 42 37" src="https://github.com/user-attachments/assets/b9d4f617-1cec-49d9-8446-68c887febbad" />

The spikes in the Message rates chart on the RabbitMQ dashboard are a direct visual representation of the activity occurring when the publisher program is executed. When we run cargo run in the publisher directory, the program connects to the broker and sends a burst of five event messages in rapid succession. This sudden influx causes a sharp increase or spike in the Publish rate, which tracks how many messages the broker is receiving per second. If the subscriber is already running and ready to receive, typically we will see a near simultaneous spike in the Deliver rate. This indicates that RabbitMQ is successfully pushing those five messages out of the queue and into the subscriber's process for handling. Because the publisher sends all five messages almost instantly and then finishes its execution, the chart shows a steep rise followed by a quick return to zero once all messages have been transmitted and processed, confirming the system has returned to an idle state.
